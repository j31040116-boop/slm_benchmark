...
| FiNER-ORD | Financial NER (token-level) | Binary classification | Accuracy, Macro-F1 | 5,000 |
...
- `pred_distribution.png` — Ground truth vs prediction distribution per task (%)
...
N_WORKERS  = 8       # parallel inference threads
MAX_TOKENS = 1536    # tokens for model output (includes thinking trace)
NUM_CTX    = 4096    # context window size (ConvFinQA uses 8192)
DEBUG_MODE = False   # True = only 3 samples per task
```

**Important:** `MAX_TOKENS` must be at least `1536` for thinking models. Lower values cause the thinking trace to consume all available tokens, leaving the model answer empty. ConvFinQA uses per-task overrides (`max_tokens: 3072`, `num_ctx: 8192`) due to long financial table inputs.
...
- **Thinking trace overhead:** LFM2.5-Thinking uses chain-of-thought before answering. Each sample takes ~20 seconds. Full benchmark (11,115 samples) took ~8 hours wall time on an RTX 2080 Ti at 8 workers.
...
**Model:** `lfm2.5-thinking:latest` · **Hardware:** RTX 2080 Ti (11GB) · **Total samples:** 11,115

| Task | n | Accuracy | Macro-F1 | Notes |
|------|---|----------|----------|-------|
| FPB | 1,000 | 0.782 | 0.203 | Accuracy inflated by class imbalance (78.5% positive) |
| FiQA-SA | 235 | 0.715 | 0.211 | Collapsed predictions |
| FiNER-ORD | 5,000 | 0.930 | 0.054 | 93% of tokens are O — model predicts O almost always |
| ConvFinQA | 1,490 | — | — | 14.8% exact match — genuine numeric reasoning difficulty |
| FOMC | 496 | 0.438 | 0.119 | Systematically confuses hawkish with dovish |
| FinSent | 506 | 0.619 | 0.327 | Best F1 of all classification tasks |
| ACL18 | 2,388 | 0.480 | 0.049 | Under-predicts neutral despite 65% ground truth neutral |

> **Key finding:** Accuracy is misleading across all tasks due to class imbalance. Macro-F1 is the reliable metric. The model shows significant prediction collapse on most tasks, with FinSent being the only classification task with meaningful F1 (0.327).
