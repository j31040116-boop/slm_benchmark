...
Outputs (saved to ./charts/):
  1. line_accuracy_vs_f1.png    — line graph: accuracy vs macro-F1 per task (gap highlighted)
  2. heatmap_metrics.png        — heatmap of all metrics across tasks
  3. pred_distribution.png      — clean per-task truth vs prediction breakdown
  4. sample_timing.png          — avg inference time per task
...
# Expected valid labels per task (for cleaning pred distribution)
VALID_LABELS = {
    ...
    "ECTSum":    {"negative", "neutral", "positive"},
...
# ── 1. LINE GRAPH — Accuracy vs Macro F1 across tasks ─────────────────────────
primaries, f1s = [], []
for t in tasks:
    m = detail[t]["metrics"]
...
ax.plot(x, [v if v is not None else np.nan for v in f1s],
        ...
        color=C_F1, label="Macro F1", zorder=3)

# Shade the gap between the two lines to highlight misleading accuracy
...
ax.set_title("LFM2.5-Thinking — Accuracy vs Macro F1 Across Tasks\n"
             "The shaded gap shows where accuracy overstates real performance",
...
# ── 3. PRED DISTRIBUTION — clean, one panel per task, skip ConvFinQA ──────────
...
for idx, t in enumerate(dist_tasks):
    ...
    # Bucket anything not in valid labels as "invalid"
    truth_counts = {l: truth_raw.get(l, 0) for l in labels}
    pred_counts  = {l: pred_raw.get(l, 0)  for l in labels}
    invalid_pred = sum(v for k, v in pred_raw.items() if k not in valid)
    if invalid_pred:
        labels_plot = labels + ["invalid"]
        truth_counts["invalid"] = 0
        pred_counts["invalid"]  = invalid_pred
    else:
        labels_plot = labels

    x     = np.arange(len(labels_plot))
    width = 0.38
    t_vals = [truth_counts[l] for l in labels_plot]
    p_vals = [pred_counts[l]  for l in labels_plot]

    b1 = ax.bar(x - width/2, t_vals, width, label="Ground truth",
                color=C_TRUE, alpha=0.82, edgecolor="white")
    b2 = ax.bar(x + width/2, p_vals, width, label="Model pred",
    ...
    for bar in list(b1) + list(b2):
        h = bar.get_height()
        if h > 0:
            ax.text(bar.get_x() + bar.get_width()/2, h + max(t_vals + p_vals) * 0.01,
...
fig.suptitle("LFM2.5-Thinking — Ground Truth vs Model Predictions by Task\n"
             "(ConvFinQA excluded — numeric task)",
...
# ── 4. TIMING — avg seconds per sample ───────────────────────────────────────
avg_times = []
for t in tasks:
    ps = detail[t].get("per_sample", [])
    avg_times.append(round(sum(s["elapsed"] for s in ps) / len(ps), 1) if ps else 0)

fig, ax = plt.subplots(figsize=(11, 5))
bars = ax.bar(tasks, avg_times, color=C_ACC, alpha=0.82, edgecolor="white", width=0.55)
for bar, val in zip(bars, avg_times):
    ax.text(bar.get_x() + bar.get_width()/2, bar.get_height() + 0.3,
            f"{val}s", ha="center", va="bottom", fontsize=11, fontweight="bold")
ax.set_ylabel("Avg seconds per sample", fontsize=12)
ax.set_title("LFM2.5-Thinking — Avg Inference Time per Task", fontsize=13, fontweight="bold")
ax.set_xticklabels(tasks, fontsize=11)
ax.yaxis.grid(True, alpha=0.4)
ax.set_axisbelow(True)
fig.tight_layout()
fig.savefig(OUT / "sample_timing.png", dpi=150)
plt.close(fig)
print("Saved: charts/sample_timing.png")
...
print(f"           Accuracy inflated by class imbalance.")
print()
